    			...
    			<div class ="card" style ="min-height:500px ; max-heihgt:1000px;">
						...
						<tbody>
								...
								<tr>
									...
									<c:if test="${vo.bdelete!=1}">
										<a href="get.do?idx=${vo.idx}&page=${pageMaker.cri.page}">${vo.title}</a>
									</c:if>
									<!-- 삭제되었습니다 제목 띄우기-->
									<c:if test="${vo.bdelete==1}">
					...
					<ul class="pagination justify-content-center">
						<c:if test="${pageMaker.prev}">
							<li class="page-item"><a class="page-link" href="list.do?page=${pageMaker.startPage-1}">Previous</a></li>
						</c:if>
					
					    <!-- 1. 페이지 List  이걸 맨처음에 한다.-->
					<!-- 1. 부트스트렙(Pagination)에 있는거 가져옵시다. [1],[2],[endPage]-->
					    <c:forEach var ="pageNum" begin ="${pageMaker.startPage}" end ="${pageMaker.endPage}">
					    	<li class="page-item ${pageMaker.cri.page==pageNum ? 'active':''}"><a class="page-link" href="list.do?page=${pageNum}">${pageNum}</a></li>
					    	
					    </c:forEach>
					    
					    <!--  다음  -->
					    <c:if test="${pageMaker.next}">
					    <li class="page-item"><a class="page-link" href="list.do?page=${pageMaker.endPage+1}">Next</a></li>
					    </c:if>
					  </ul>
