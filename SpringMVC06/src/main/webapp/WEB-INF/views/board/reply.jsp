  ...
  $(document).ready(function(){
	  $("button").on("click",function(e){
		  ...
		  if(a=="update"){
			  ...
			  frm.attr("action","reply.do");  //답글달기
			  frm.attr("method","post")
			  frm.submit();
		  }
		  //부모글의 idx가 넘어간다고 생각해라? 난 내께 id가 보내지는걸로 이해했다.
...
<div class="card">
	...
	<div class ="card" style ="min-height:500px ; max-heihgt:1000px;">
	  <div class="card">
	    <div class="card-header">답변달기</div>
	    <div class="card-body">
	    	<form id="frm">
	    		<!-- 폼이 전달될때 값을 넘길려고 쓴다. -->
	    		...
	    		<input type = "hidden" name="keyword" value ="${cri.keyword}"/>
						<div class="form-group">
							...
							<input type="text" value= "${vo.title}"  class="form-control" placeholder="Enter title" name= "title">
						</div>
						<div class ="form-group">
						...
						<div class="form-group">
							<label for="writer">작성자:</label> <input type="text" value= "${m.memName}"
						...
						<button type="button" data-oper="reply" class = "btn btn-sm info"   ${!empty m ? '' : "disabled='disabled'"}>답글저장</button>
						<button type="button" data-oper="list" class="btn btn-sm btn-success">목록</button>
						<button type="button" data-oper='reset' class ="btn btn-sm btn-sucess">취소</button>
					</form>
	    </div> 
